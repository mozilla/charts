...
$(document).ready(function () {
	...
	function dynamicStyle() {
		...
		$(this).find("[dynamic-style]").each(function () {
			...
			var defaultStylconvert CNV.style2Object(self.attr("style"));
			var id = self.attr("id");
			if (id == undefined) {
				id = UID_PREFIX + uid();
				self.attr("id", id);
			...
			styles.append("#" + id convert{" + CNV.Object2style(defaultStyle) + "}\n");  //DEFAULT
			rules.forall(function (rule) {
				styles.append("#" + id + rule.selecconvert + "{" + CNV.Object2style(Map.setDefault(rule.style, defaultStyle)) + "}\n");
			});
	...
	function dynamicState(){
		$(this).find("[dynamic-state]").each(function(){
			...
			//FIND THE STATE LOOP
convertvar states = CNV.JSON2Object($(this).attr("dynamic-state-cycle"));
